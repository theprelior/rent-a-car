 ...
 const router = useRouter();
  ...
  return (
    <form onSubmit={handleSearch} className="mx-auto mt-10 w-full max-w-4xl rounded-2xl bg-gray-900/50 p-6 shadow-xl backdrop-blur-lg">
      ...
      {/* İSTEDİĞİNİZ YENİ 2x2'LİK YAPI */}
      <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4">
        
        {/* Satır 1, Sütun 1: Alış Lokasyonu */}
        <div>
        ...
        {/* Satır 1, Sütun 2: Bırakış Lokasyonu */}
        <div>
        ...
        {/* Satır 2, Sütun 1: Alış Tarih & Saat */}
        <div>
            <label className="mb-2 block text-sm font-semibold text-white">Alış Tarihi</label>
            <div className="flex gap-2">
                <div className="relative w-3/5"><IconCalendar /><input value={startDate} onChange={(e) => setStartDate(e.target.value)} type="date" required className="w-full rounded-md border-gray-600 bg-gray-700/50 py-3 pl-11 pr-2 text-white focus:border-blue-500 focus:ring-blue-500 [color-scheme:dark]" /></div>
                <div className="relative w-2/5"><IconClock /><input value={startTime} onChange={(e) => setStartTime(e.target.value)} type="time" required className="w-full rounded-md border-gray-600 bg-gray-700/50 py-3 pl-11 pr-2 text-white focus:border-blue-500 focus:ring-blue-500 [color-scheme:dark]" /></div>
            </div>
        </div>
        
        {/* Satır 2, Sütun 2: Bırakış Tarih & Saat */}
        <div>
            <label className="mb-2 block text-sm font-semibold text-white">Bırakış Tarihi</label>
            <div className="flex gap-2">
                <div className="relative w-3/5"><IconCalendar /><input value={endDate} onChange={(e) => setEndDate(e.target.value)} type="date" required className="w-full rounded-md border-gray-600 bg-gray-700/50 py-3 pl-11 pr-2 text-white focus:border-blue-500 focus:ring-blue-500 [color-scheme:dark]" /></div>
                <div className="relative w-2/5"><IconClock /><input value={endTime} onChange={(e) => setEndTime(e.target.value)} type="time" required className="w-full rounded-md border-gray-600 bg-gray-700/50 py-3 pl-11 pr-2 text-white focus:border-blue-500 focus:ring-blue-500 [color-scheme:dark]" /></div>
            </div>
        </div>
      </div>

      {/* Buton artık grid'in dışında, kendi başına */}
