...
export function Navbar() {
  ...
  return (
    <>
      <header className={`fixed top-0 z-50 w-full transition-all duration-300 ${isScrolled ? "bg-black/95 shadow-lg backdrop-blur-md" : "bg-black/70"}`}>
        <div className="container mx-auto flex h-16 items-center justify-between px-4">
          <Link
            href="/"
            className="text-3xl font-extrabold tracking-wider bg-gradient-to-r from-yellow-400 to-yellow-600 bg-clip-text text-transparent animate-fade-in-down"
          >
            RENTORA
          </Link>
            ...
            {navItems.map((item) => {
              ...
              return (
                <Link
                  key={item.name}
                  href={item.href}
                  className={`rounded-full px-4 py-2 text-base font-semibold transition-all duration-200 ease-in-out transform ${ // duration ve transform eklendi
                    isActive
                      ? "bg-yellow-600 text-black shadow-md" // Aktif olana da gölge ekleyelim
                      : "text-gray-300 hover:text-yellow-400 hover:bg-neutral-800 hover:translate-y-[-2px] hover:shadow-lg" // Hover efektleri
                    }`}
                >
                  {item.name}
                </Link>
            ...
            <div className="hidden items-center space-x-4 lg:flex">
              {status === "authenticated" ? (
                <>
                  <Link
                    href="/profil"
                    className="flex items-center space-x-2 text-base text-gray-200 hover:text-yellow-400 transition-all duration-200 ease-in-out transform hover:translate-y-[-2px]"
                  >
                    <IconUser />
                ...
                <>
                  <Link
                    href="/auth/login"
                    className="text-base font-medium text-gray-200 hover:text-yellow-400 transition-all duration-200 ease-in-out transform hover:translate-y-[-2px]"
                  >
                    Üye Girişi
                  </Link>
                  <Link
                    href="/auth/register"
                    className="rounded-full bg-gradient-to-r from-yellow-400 to-yellow-600 px-5 py-2.5 text-base font-semibold text-black shadow-lg transition-all duration-200 ease-in-out transform hover:scale-105 hover:translate-y-[-2px]"
                  >
                    Kayıt Ol
                  </Link>
                </>
      ...
      <div className={`fixed top-20 z-40 w-full bg-black/95 transition-transform duration-300 lg:hidden ${isMobileMenuOpen ? "translate-y-0" : "-translate-y-[120%]"}`}>
        <nav className="container mx-auto flex flex-col space-y-1 p-4">
          ...
          <div className="border-t border-neutral-700 pt-4 mt-4">
            {status === "authenticated" ? (
              ...
              <div className="space-y-4">
                <Link href="/auth/login" className="flex items-center space-x-2 rounded-md px-3 py-3 text-base font-semibold text-gray-300 hover:text-yellow-400 hover:bg-neutral-800">
                  <IconUser />
                  <span>Üye Girişi</span>
                </Link>
                <Link href="/auth/register" className="flex w-full items-center justify-center rounded-full bg-gradient-to-r from-yellow-400 to-yellow-600 px-3 py-3 text-base font-semibold text-black shadow-md hover:scale-105 transition-transform">
                  Kayıt Ol
                </Link>
              </div>
            )}
